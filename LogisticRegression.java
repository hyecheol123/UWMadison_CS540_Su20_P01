...
public class LogisticRegression {
   ...
   *             0. Question 1 saved file name (saveFeatureVector)
   *             1. Question 2 saved file name (saveModel)
   *             2. Question 3 saved file name (saveTestActivation)
   *             3. Question 4 saved file name (saveTestPrediction)
   *             4. log file
   */
  public static void main(String[] args) {
    ...
    // Testing the model
    Double[] testActivation = calculateActivation(dataset.getTestingFeatures()); // calculate activation for testing set
  ...
  /**
   * Method to handle Question 1 - save one of the training sample as text (CSV) file
   *
   ...
   *
   * @param features ArrayList<Double[]> array contains list of features (data point)
  ...
  private static void updateWeightsAndBias(Double[] activation) {
    ...
    for(int featureIndex = 0; featureIndex < FEATURE_DIMENSION; featureIndex++) {
      ...
      for(int dataIndex = 0; dataIndex < dataset.getTrainingFeatures().size(); dataIndex++) {
        updateWeights += (activation[dataIndex] - dataset.getTrainingLabels().get(dataIndex)) // (a_i - y_i) * x_i
  ...
  private static boolean checkConvergence(Double[] activation) {
    ...
    // Check for convergence (Return true when abs(previousLoss - currentLoss) is less than epsilon)
